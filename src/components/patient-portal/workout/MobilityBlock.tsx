...
export function MobilityBlock({ sessions, onOpen }: MobilityBlockProps) {
  ...
  return (
    <div className="rounded-2xl border border-violet-200 bg-gradient-to-br from-violet-50 via-white to-white p-3 shadow-sm">
      ...
      <div className="space-y-2">
        {sessions.map((s) => (
          <button
            key={s.id}
            onClick={() => onOpen(s.id)}
            className="flex w-full items-center gap-3 rounded-xl border border-violet-200 bg-white p-2.5 text-left shadow-sm transition hover:border-violet-400 hover:shadow-md"
          >
