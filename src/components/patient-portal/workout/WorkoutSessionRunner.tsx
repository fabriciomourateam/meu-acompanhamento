  ...
  return (
    <div className="space-y-4">
      ...
      <motion.div
        ...
        className="rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 shadow-sm p-4"
      >
        <div className="flex items-start justify-between gap-3">
          <div className="flex-1 min-w-0">
            <p className="text-[10px] uppercase tracking-wider text-emerald-600 dark:text-emerald-400 font-semibold">Sessão · {plan.name}</p>
            <h2 className="text-xl font-bold leading-tight text-slate-900 dark:text-slate-100">{session.name}</h2>
            {session.focus && session.focus.trim().toLowerCase() !== session.name.trim().toLowerCase() ? (
              <p className="text-sm text-slate-600 dark:text-slate-400">{session.focus}</p>
            ) : null}
          </div>
          <div className="flex flex-col items-end gap-1 shrink-0">
            <div className="flex items-center gap-1 bg-emerald-50 dark:bg-emerald-950/40 text-emerald-700 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-900/50 rounded-full px-2.5 py-1 text-xs font-semibold">
            ...
            {sessionLogId && startedAt ? (
              ...
              <div
                className="flex items-center gap-1 text-[11px] font-medium tabular-nums text-slate-400 dark:text-slate-500"
                title="Tempo decorrido desde que você começou o treino (só informativo)"
              >
                <Clock className="w-3 h-3" />
                <span className="font-normal">Tempo de treino</span>{' '}
                <span className="font-semibold text-slate-500 dark:text-slate-400">{formatDuration(elapsedSec)}</span>
              </div>
            ) : null}
          </div>
        </div>
        ...
        <div className="mt-3">
          ...
          <p className="mt-1 text-[11px] text-slate-400 dark:text-slate-500">{progressPct}% concluído</p>
        </div>

        {sessionLogId ? (
          <>
            <div className="mt-3 flex items-center justify-between text-sm">
              <span className="text-slate-600 dark:text-slate-400">Volume: <strong className="tabular-nums text-slate-900 dark:text-slate-100">{totalVolume.toFixed(0)} kg</strong></span>
            ...
            <button
              ...
              className="mt-2 flex items-center gap-1 text-xs font-medium text-slate-400 dark:text-slate-500 transition-colors hover:text-rose-500"
            >
      ...
      {reorderMode ? (
        <div className="space-y-2">
          <p className="px-1 text-xs text-slate-500 dark:text-slate-400">Ajuste a ordem do treino de hoje (ex.: aparelho ocupado). Vale só pra esta sessão.</p>
          {orderedExercises.map((ex, idx) => (
            <div key={ex.id} className="flex items-center gap-2 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-2.5">
              <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-slate-100 dark:bg-slate-800 text-xs font-bold text-slate-600 dark:text-slate-400">{idx + 1}</span>
              <span className="min-w-0 flex-1 truncate text-sm font-medium text-slate-800 dark:text-slate-200">{subs[ex.id]?.name ?? ex.exercise_name}</span>
              <Button size="sm" variant="outline" disabled={idx === 0} onClick={() => moveExercise(ex.id, -1)} className="h-9 w-9 p-0 border-slate-200 dark:border-slate-700" aria-label="Subir">
                <ArrowUp className="h-4 w-4" />
              </Button>
              <Button size="sm" variant="outline" disabled={idx === orderedExercises.length - 1} onClick={() => moveExercise(ex.id, 1)} className="h-9 w-9 p-0 border-slate-200 dark:border-slate-700" aria-label="Descer">
      ...
      <AlertDialog open={confirmReset} onOpenChange={(v) => !resetting && setConfirmReset(v)}>
        <AlertDialogContent className="bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-700">
          <AlertDialogHeader>
            <AlertDialogTitle className="text-slate-800 dark:text-slate-200">Descartar este treino?</AlertDialogTitle>
