...
// Formata a quantidade do alimento incluindo a gramatura da medida caseira
// quando houver (ex.: "2 unidade (220g)").
function fmtQty(food: any): string {
  ...
  if (g > 0 && !isBase && food.quantity != null) {
    return `${base} (${Math.round(food.quantity * g)}g)`;
  }
  return base;
}

// Linha de substituições de um alimento (ex.: "🔁 Substituir por: 25g de Queijo · 45g de Frango").
...
// Card de 1 alimento: nome, quantidade (+medida caseira), macros, observação e substituições.
function foodCardHtml(food: any): string {
  const macros = (food.protein || food.carbs || food.fats)
    ? `<span style="color:#64748b;"> · P ${food.protein ?? 0} C ${food.carbs ?? 0} G ${food.fats ?? 0}</span>` : '';
  return `
    <div style="background:rgba(15,23,42,0.5);border:1px solid rgba(71,85,105,0.3);border-radius:10px;padding:12px 16px;margin-bottom:8px;">
      <div style="display:flex;justify-content:space-between;align-items:center;gap:12px;">
        ...
        <span style="font-size:13px;color:#94a3b8;font-weight:500;white-space:nowrap;text-align:right;">${food.calories ? `${food.calories} kcal` : ''}${macros}</span>
      </div>
      ${food.notes ? `<div style="margin:6px 0 0 20px;font-size:12px;color:#94a3b8;font-style:italic;">📝 ${food.notes}</div>` : ''}
      ${foodSubsHtml(food)}
    </div>`;
}
...
export class DietPDFGenerator {
  static async generatePDF(
    ...
    // Totais somam SÓ as refeições principais — a opção é substituta ("coma a
    // principal OU a opção"), então não pode dupla-contar.
    ...
    const renderMeal = (meal: any, isOption: boolean): string => {
      ...
      // Refeição-OPÇÃO: aninhada, com badge e a semântica "ou".
      if (isOption) {
        return `
          <div style="margin-top:12px;border:1px dashed rgba(245,158,11,0.55);border-radius:12px;padding:16px;background:rgba(245,158,11,0.06);">
            <div style="display:flex;align-items:center;gap:10px;margin-bottom:12px;flex-wrap:wrap;">
              <span style="background:rgba(245,158,11,0.2);color:#fbbf24;padding:3px 10px;border-radius:8px;font-size:11px;font-weight:700;">🔁 OPÇÃO</span>
              <span style="font-size:16px;font-weight:700;color:#fcd34d;">${meal.meal_name}</span>
              <span style="font-size:11px;color:#94a3b8;font-style:italic;">— coma a principal OU esta opção</span>
            </div>
            ${macrosHtml}
      ...
      return `
        <div style="background:rgba(30,41,59,0.6);border:1px solid rgba(71,85,105,0.5);border-radius:16px;padding:24px;margin-bottom:16px;page-break-inside:avoid;">
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;padding-bottom:12px;border-bottom:1px solid rgba(71,85,105,0.5);">
            ...
            ${time ? `<span style="background:rgba(99,102,241,0.15);color:#a5b4fc;padding:6px 12px;border-radius:20px;font-size:13px;font-weight:500;border:1px solid rgba(99,102,241,0.3);">${time}</span>` : ''}
          </div>
          ${macrosHtml}
          <div style="display:flex;flex-direction:column;gap:8px;">${foodsHtml}</div>
          ${instructionsHtml}
    ...
    // Separa SUPLEMENTAÇÃO das demais orientações (diferenciadas por guideline_type).
    ...
    const guidelineSection = (titulo: string, icon: string, items: any[]) => items.length === 0 ? '' : `
      <div style="margin-top:32px;padding-top:24px;border-top:2px solid rgba(71,85,105,0.5);">
        <div style="font-size:20px;font-weight:700;color:#ffffff;margin-bottom:16px;display:flex;align-items:center;gap:10px;">${icon} ${titulo}</div>
