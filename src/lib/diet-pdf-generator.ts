...
//  - food.notes = '{"substitutions":[...]}' (legado de import) — sem tratar,
//    vinha como "código" cru no PDF.
...
// Linha de substituições (nome + quantidade, sem macros).
...
function foodCardHtml(food: any): string {
  ...
  return `
    <div style="background:rgba(15,23,42,0.5);border:1px solid rgba(71,85,105,0.3);border-radius:10px;padding:12px 16px;margin-bottom:8px;">
      ...
      ${notes ? `<div style="margin:6px 0 0 20px;font-size:12px;color:#94a3b8;font-style:italic;">📝 ${notes}</div>` : ''}
      ${subsLineHtml(subs)}
    </div>`;
}
